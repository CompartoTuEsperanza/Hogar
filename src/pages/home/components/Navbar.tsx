  ...
  return (
    <>
      <nav
        ...
        style={{ background: scrolled ? 'rgba(117,14,32,0.97)' : 'transparent' }}
      >
        <div className="w-full px-4 h-16 flex items-center justify-between gap-3">
            ...
            {links.map((l) => (
              <li key={l.key}>
                <button
                  ...
                  className="text-cream/80 hover:text-gold transition-colors uppercase font-bold cursor-pointer text-center leading-tight" style={{ fontFamily: "'Josefin Sans', sans-serif", fontSize: "0.62rem", letterSpacing: "0.1em", whiteSpace: "nowrap" }}
                >
                  {l.label}
                </button>
              </li>
          ...
          <div ref={langRef} className="relative hidden md:block">
            ...
            {langOpen && (
              <div className="absolute right-0 top-full mt-2 border border-gold/20 rounded-lg overflow-hidden shadow-xl min-w-[140px]" style={{ background: "#750E20" }}>
                {LANGUAGES.map((lang) => (
                  <button
                    key={lang.code}
          ...
          <div className="md:hidden flex items-center gap-2">
            ...
            <div ref={mobileLangRef} className="relative">
              ...
              {mobileLangOpen && (
                <div className="absolute right-0 top-full mt-2 border border-gold/20 rounded-xl overflow-hidden z-[60]" style={{ minWidth: 130, boxShadow: '0 12px 40px rgba(0,0,0,0.5)', background: "#750E20" }}>
                  {LANGUAGES.map((lang) => (
                    <button
                      key={lang.code}
                      onClick={() => changeLanguage(lang.code)}
      ...
      <div className={`fixed inset-0 z-50 transition-all duration-300 md:hidden ${mobOpen ? 'visible opacity-100' : 'invisible opacity-0'}`}>
        ...
        <div className={`absolute top-0 right-0 w-72 h-full transition-transform duration-300 ${mobOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col p-8`} style={{ background: "#750E20" }}>
